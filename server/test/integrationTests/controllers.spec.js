...
describe("Integration Test", function () {
  const setup = async () => {
    ...
    const choreId = savedTask._id;
    newUser.chores.push(savedTask._id);
    await newUser.save();
    return { token, choreId };
  };
  const tearDown = async () => {
    await User.deleteOne({ email: "user1@iamarealuser.com" });
    await Task.deleteMany({ name: "Sweep the floor" });
  };
  ...
  describe("DELETE /api/chores/:id", function () {
    it("should delete an existing chore", async () => {
      ...
      const chore = await Task.findOne({ name: "Sweep the floor" });
      console.log(chore);
  ...
  describe("POST /api/chores/", function () {
    it("should create a new chore", async () => {
      ...
      const res = await chai
        ...
        .send(body);
      expect(res.status).to.equal(201);
      expect(res.body.Task.name).to.equal("Clean the windows");
      expect(res.body.Task.location).to.equal("Room");
      expect(res.body.Task.duration).to.equal(3000);
      expect(res.body.Task.preference).to.equal("high");
      expect(res.body.Task.frequency.quantity).to.equal(2);
      expect(res.body.Task.frequency.interval).to.equal("weeks");
      await Task.deleteOne({ name: "Clean the windows" });
    });
  ...
  describe("GET /api/schedule/", () => {
    ...
    after(async function () {
      ...
      await Task.deleteMany({ location: "TestLocation" });
    });
  ...
  describe("PUT /api/chores/:id/checked/", function () {
    ...
    it("should return a 404 error for checking off a chore with an invalid id", async function () {
      ...
      expect(res.body).to.deep.equal({
        message: "Task with id 640ab1e7823cae42437b3c92 was not found.",
      });
    });
  });
});
