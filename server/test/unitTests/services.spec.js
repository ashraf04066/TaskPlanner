...
describe("startEmailService", function () {
  ...
  let testUsers = [
    { email: "user1@test.com", chores: [], receiveNotifs: true },
    {
      ...
      chores: [testTasks[0], testTasks[1], testTasks[2], testTasks[3]],
      receiveNotifs: true,
    },
    { email: "user3@test.com", chores: [testTasks[2]], receiveNotifs: true },
    { email: "user4@test.com", chores: [testTasks[2]], receiveNotifs: false },
  ];
  ...
  beforeEach(function () {
    ...
    let pos = 0;
    // Stub the User and Task models
    userFindStub = sandbox.stub(User, "find").resolves(testUsers);
    choreFindStub = sandbox.stub(Task, "find").callsFake(() => {
      const userTasks = testUsers[pos].chores;
      pos += 1;
  ...
  it("should return an error if there is an exception thrown before sending the mail", async function () {
    ...
    // Assert that the User and Task models were called with the correct arguments
  ...
  it("should send an email to each user with their chores for the day", async function () {
    ...
    expect(Task.find).to.have.been.calledThrice;
    expect(Task.find).to.have.been.calledWith({
      _id: { $in: [] },
      nextOccurrence: { $exists: true },
    });
    expect(Task.find).to.have.been.calledWith({
      _id: {
        $in: [testTasks[0], testTasks[1], testTasks[2], testTasks[3]],
      },
      nextOccurrence: { $exists: true },
    });
    expect(Task.find).to.have.been.calledWith({
  ...
  it("should return correct chores for today and overdue chores", async () => {
    const res0 = await getTasksForUser(testUsers[0]);
    expect(res0.todaysTasks).to.deep.equal([]);
    expect(res0.overdueTasks).to.deep.equal([]);
  });
