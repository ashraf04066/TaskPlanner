...
describe("Testing Tasks controllers", () => {
  describe("createTask", () => {
    let req;
    let res;
    let sandbox;
    ...
    it("returns 409 status code if chore already exists", async () => {
      ...
      sandbox.stub(Task, "findOne").returns({});
      ...
      expect(
        res.json.calledWith({
          message:
            "Task: Clean the kitchen already exists. If you want to change somthing, please use the EDIT option.",
        })
      ).to.be.true;
    });
    ...
    it("returns 500 status code if an error occurs while saving chore", async () => {
      ...
      sandbox.stub(Task, "findOne").returns(null);

      // Testing error handling
      sandbox.stub(Task.prototype, "save").throws();
      await createTask(req, res);
      expect(res.status.calledWith(500)).to.be.true;
      expect(res.json.calledWith({ message: "Task could not be created" })).to
        .be.true;
    });
  ...
  describe("getAllTasks", () => {
    ...
    it("should return all chores for a user", async () => {
      ...
      // Create a stub for the Task model
      const choreStub = sinon.stub(Task, "find");
  ...
  describe("editTask", () => {
    ...
    beforeEach(() => {
      ...
      findOneStub = sinon.stub(Task, "findOne");
      sandbox = sinon.createSandbox();
      sandbox.stub(Task.prototype, "save").returns({
        _id: "chore123",
        name: "Clean the kitchen",
  ...
  describe("getSingleTask", () => {
    ...
    it("should return a 200 response with the chore if it is found", async () => {
      ...
      sinon.stub(Task, "findOne").resolves(chore);
    ...
    it("should return a 500 response with an error message if there is an unexpected error", async () => {
      const error = new Error("Unexpected error");
      sinon.stub(Task, "findOne").rejects(error);
  ...
  describe("deleteTask", () => {
    ...
    beforeEach(() => {
      ...
      findByIdAndDelete = sandbox.stub(Task, "findByIdAndDelete");
  ...
  describe("checkOffTask", () => {
    ...
    it("should return a 201 response with the chore if it is found", async () => {
      ...
      await checkOffTask(req, res);

      expect(res.status.calledWith(201)).to.be.true;
      expect(
        res.json.calledOnceWith({ message: `Task checked off successfully!` })
      ).to.be.true;
    });
