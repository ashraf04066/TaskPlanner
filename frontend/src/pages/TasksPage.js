...
const TasksPage = ({ isTasksLoading, choresData }) => {
  const MIN_TO_SEC = 60;
  ...
  const [currTask, setCurrTask] = useState(null); // Current chore being updated
  ...
  const handleTasks = () => queryClient.invalidateQueries("chores");
  ...
  const columns = useMemo(
    ...
    [choresData, isTasksLoading]
  );

  // Task data and table setup
  const data = useMemo(formatTaskData, [choresData, isTasksLoading]);
  const {
    getTableProps,
    getTableBodyProps,
    headerGroups,
    ...
    <>
      ...
      {isTasksLoading ? (
        ...
        <div className="content">
          ...
          <Dialog
            ...
            footer={
              <>
                ...
                <Button
                  onClick={() => {
                    deleteTaskMutation.mutate(currTask._id);
          ...
          >
            {currTask != null && (
              <p>Are you sure you want to delete chore {currTask.name}?</p>
            )}
